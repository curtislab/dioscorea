function dioscoreaFooter() {
	document.write(' \
		<div id="footer-wrap"> \
		  <a href="http://curtislab.org">A CurtisLab Production</a>  \
                  on <a href="http://github.com/curtislab/dioscorea">GitHub</a> \
		  <a href="http://creativecommons.org/licenses/by-sa/4.0/">under a  \
		  Creative Commons Attribution-ShareAlike 4.0 License</a></p> \
		</div> \
		')
	}
	
function dioscoreaStub() {
	document.write(' \
	  <h1>dioscorea.org</h1> \
    <img src="img/Dioscorea_balcanica_BotGardBln310505.jpg" style="float:right;"> \
		<p> \
		  This page is currently under construction. In the meantime, go play in soil,   \
		  <a href="http://discovermagazine.com/2007/jul/raw-data-is-dirt-the-new-prozac">the new Prozac</a>!  \
		</p> \
		')
}

function dioscoreaNav() {
	document.write(' \
		  <nav id="primary_nav_wrap"> \
		    <ul> \
		      <li class="nav"><a href=".">Home</a> \
		      </li> \
		      <li><a href="#">About Dioscorea</a> \
			<ul> \
			  <li><a href="basics.html">Basics</a></li> \
			  <li><a href="taxonomy.html">Taxonomy</a></li> \
			  <li><a href="phylogeny.html">Phylogeny</a></li> \
			  <li><a href="genetics.html">Genetics</a></li>  \
			  <li><a href="nutrition.html">Nutrition</a></li> \
			  <li><a href="medical.html">Medical Uses</a></li> \
			</ul> \
		      </li> \
		      <li><a href="#">Yam Propagation 101</a> \
			<ul> \
			  <li><a href="where.html">Climate & Regions</a></li> \
			  <li><a href="techniques.html">Techniques</a></li> \
			  <li><a href="economics.html">Economics</a></li> \
			  <li><a href="gender.html">Gender Roles</a></li> \
			  <li><a href="challenges.html">Challenges</a></li> \
			</ul> \
		      </li> \
		      <li><a href="#">Resources</a> \
		      	<ul> \
		      	  <li><a href="researchResources.html">For Researchers</a></li> \
		      	  <li><a href="farmerResources.html">For Sharecroppers</a></li> \
		      	  <li><a href="media.html">Media Recipes</a></li> \
		      	  <li><a href="SOPs.html">Protocols</a></li> \
		      	</ul> \
		      </li> \
		      <li><a href="#">Research</a> \
			<ul> \
			  <li><a href="researchers.html">Research Groups</a></li> \
			  <li><a href="centers.html">Centers & Initiatives</a></li> \
			  <li><a href="publications.html">Publications</a></li> \
			  <li><a href="projects.html">Projects</a></li> \
			  <li><a href="ongoing.html">Ongoing Research</a></li> \
			  	<ul> \
			  	<li><a href="breeding.html">Breeding (In)Efficiency</a></li> \
			  	<li><a href="biodiversity.html">Biodiversity</a></li> \
			  	<li><a href="VCA-econ.html">Economics/Value Chain Analysis</a></li> \
			  	<li><a href="foodScience.html">Food Science & Value Addition</a></li> \
			  	<li><a href="genetics.html">Genetics & Ploidy Levels</a></li> \
			  	<li><a href="macropropagation.html">Macropropagation</a></li> \
			  	<li><a href="micropropagation.html">Micropropagation</a></li> \
			  	  <ul> \
			  	  <li><a href="se.html">Somatic Embryogenesis</a></li> \
			  	  <li><a href="bioreactors.html">Temporary Immersion Reactors</a></li> \
			  	  </ul> \
			  	<li><a href="viruses.html">Viruses & Pathogens</a></li> \
			  	<li><a href="storage.html">Shelf-Life/Storage</a></li> \
			  	<li><a href="germplasm.html">Germplasm Conservation</a></li> \
			  	<li><a href="techTransfer.html">Technology Transfer</a></li> \
			  	</ul> \
			</ul> \
		      </li> \
		      <li><a href="#">Multimedia</a> \
			<ul> \
			  <li><a href="recipes.html">Recipes</a></li> \
			  <li><a href="gallery.html">Gallery</a></li> \
			  <li><a href="https://www.youtube.com/channel/UCLy61dmjtRfL6e1OMeRZT4A/feed">Videos</a></li> \
			  <li><a href="external.html">More Resources</a></li> \
			</ul> \
		      </li> \
		      <li><a href="contact.html">Contact</a> \
		      </li> \
		    </ul> \
		  </nav> \
		')
	}
